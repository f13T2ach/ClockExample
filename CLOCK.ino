...
const char ssid[] = "Wireless_ABCD";  //Wifi账号
const char pass[] = "PasswordOfWlan"; //WIFI密码
...
void setup()
{
  ...
  TJpgDec.drawJpg(0,135,watchbottom, sizeof(watchbottom));
  ...
  TJpgDec.drawJpg(165,35,temperature, sizeof(temperature));  //温度图标
  TJpgDec.drawJpg(163,92,humidity, sizeof(humidity));  //湿度图标
...
void drawBoxes(){
  ...
  tft.drawFastHLine(0,90,240,TFT_BLACK);
  
  tft.drawFastVLine(60,90,34,TFT_BLACK);
  tft.drawFastVLine(160,90,34,TFT_BLACK);
}
 ...
 String URL = "http://wgeo.weather.com.cn/ip/?_="+String(now());
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if(aa>-1){
       ...
       if(str.substring(aa+4,aa+4+9)==cityCode||cityCode==""){
...
void weaterData(String *cityDZ,String *dataSK,String *dataFC){
  ...
  clk.drawString(sk["temp"].as<String>()+"°",40,14);
  clk.pushSprite(175,34);
  clk.deleteSprite();
  ...
  String aqiTxt = "优";
  int pm25V = sk["aqi"];
  //uint16_t pm25BgColor = tft.color565(156,130,127);//优
  if(pm25V>200){
...
void digitalClockDisplay()
{
  ...
  clk.setTextDatum(CC_DATUM);
  clk.setTextColor(TFT_BLACK, bgColor);  
  clk.drawString(monthDay(),49,14);
  clk.pushSprite(61,89);
  clk.deleteSprite();
